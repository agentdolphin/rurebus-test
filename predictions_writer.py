...
from predictions_tagrel_writer import check_output_dir, read_examples, write_tag_ann_to_file
...
def get_tag_dataset(rurebus_data_dir: str, part: str = 'train_part_3'):
    ...
    annotatated_dataset = examples_generator.get_annotated_dataset(rurebus_dir=rurebus_dir)
    tokenized_dataset = examples_generator.get_tokenized_dataset(rurebus_dir=rurebus_dir)
    ...
    for annotated_file in dataset:
        ...
        for sent_id, (sentence, tokenized_sentence) in enumerate(zip(sentences, tokenized_sentences)):
            entities_from_span = examples_generator.filter_entities_dict_by_text_span(
            ...
            for token in tokenized_sentence:
                tag, entity_tag = examples_generator.get_entity_tag_from_token_span(entities_from_span,
