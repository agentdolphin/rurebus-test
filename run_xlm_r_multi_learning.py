...
from xlm_r_for_multilearning import XLMRMultiLearning
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, sent_type_id, tag_id, relation_id, orig_positions_map=None):
...
def convert_examples_to_features1(examples, label2id, max_seq_length, tokenizer, do_filter_tags=False,
                                  repeat_relations=None):
    ...
    neg_tag_label = 'O'
    neg_rel_label = '0'
    SUBJECT_START = "madeupword0000"
    SUBJECT_END = "madeupword0000"
    SENTENCE_START = "madeupword0001"
    SENTENCE_END = "madeupword0001"
    CLS, SEP = '<s>', '</s>'
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for i, (token, tag, relation) in enumerate(zip(example.sentence, example.tag, example.relation)):
            ...
            if offset < 255:
                ...
                if additional_offset + offset < 255:
            ...
            cur_tag = tag
            if do_filter_tags:
                cur_tag = tag if tag in EVAL_TAGS else 'O'

            tag_labels += [cur_tag] * num_sub_tokens
            relation_labels += [relation] * num_sub_tokens
        ...
        input_ids = tokenizer.encode(tokens, add_special_tokens=False, truncation_strategy='do_not_truncate')
        ...
        try:
            ...
            tag_id = [label2id['tag'][tag] for tag in tag_labels]
            relation_id = [label2id['relation'][relation] for relation in relation_labels]
        except KeyError:
            print(label2id['tag'], tag_labels)
            print(label2id['relation'], relation_labels)
            print(label2id['sent_type'], example.sent_type)
            raise KeyError

        assert len(input_ids) == max_seq_length, f"{len(input_ids)}"
        assert len(input_mask) == max_seq_length, f"{len(input_mask)}"
        assert len(segment_ids) == max_seq_length, f"{len(segment_ids)}"
        assert len(tag_id) == max_seq_length, f"{len(tag_id)}"
        assert len(relation_id) == max_seq_length, f"{len(relation_id)}"
        ...
        if num_shown_examples < 20:
            if (ex_index < 5) or (any([x != 0 for x in relation_id])):
                ...
                logger.info("orig_to_sub_map: %s" % " ".join([str(x) for x in orig_positions_map]))
        ...
        if repeat_relations is not None:
            relations_to_repeat, repeat_time = repeat_relations
            if any([rel in relations_to_repeat for rel in example.relation]):
                features.extend([InputFeatures(
                    input_ids=input_ids,
                    input_mask=input_mask,
                    segment_ids=segment_ids,
                    sent_type_id=sent_type_id,
                    tag_id=tag_id,
                    relation_id=relation_id,
                    orig_positions_map=orig_positions_map)] * (repeat_time - 1))
...
def simple_accuracy(tag_preds, tag_labels, relation_preds, relation_labels):
    return {
...
def compute_f1(tag_preds, tag_labels, relation_preds, relation_labels, label2id):
    return {
...
def evaluate(model, device, eval_dataloader,
    ...
    model.eval()
    if hasattr(model, 'tag_loss_weight'):
    ...
    for input_ids, input_mask, segment_ids, tag_label_ids, \
        ...
        tag_logits, relation_logits = output[:2]
        # print(tag_logits.shape, tag_label_ids.shape)
    ...
    print(preds['relation'][0].shape)
    preds['tag'] = np.concatenate(preds['tag'], axis=0)
    preds['relation'] = np.concatenate(preds['relation'], axis=0)
    print(preds['relation'].shape)
    scores = {}
    if compute_scores:
    ...
    for key in preds:
        scores[key] = np.max(softmax(preds[key], axis=-1), axis=-1)
        preds[key] = np.argmax(preds[key], axis=-1)

    if compute_scores:
        result = compute_f1(np.array([x for y in preds['tag'] for x in y]),
                            ...
                            np.array([x for y in eval_relation_ids.numpy() for
                                      x in y]), label2id)
        result.update(simple_accuracy(np.array([x for y in preds['tag'] for x in y]),
                                      np.array([x for y in eval_tag_ids.numpy() for x in y]),
                                      np.array([x for y in preds['relation'] for x in y]),
                                      np.array([x for y in eval_relation_ids.numpy() for
                                                x in y])))
...
def main(args):
    ...
    eval_dataloader, eval_sent_type_ids = [], []
    eval_tag_ids, eval_relation_ids = [], []
    ...
    relation_label_list = processor.get_relation_labels1(args.data_dir)
    ...
    label2id = {'sent_type': {label: i for i, label in enumerate(sent_type_label_list)},
                'tag': {label: i for i, label in enumerate(tag_label_list)},
                'relation': {label: i for i, label in enumerate(relation_label_list)}}

    id2label = {'sent_type': {i: label for i, label in enumerate(sent_type_label_list)},
                'tag': {i: label for i, label in enumerate(tag_label_list)},
                'relation': {i: label for i, label in enumerate(relation_label_list)}}

    num_tag_labels = len(tag_label_list)
    num_relation_labels = len(relation_label_list)

    do_lower_case = 'uncased' in args.model
    tokenizer = XLMRobertaTokenizer.from_pretrained(args.model, do_lower_case=do_lower_case)
    ...
    if args.do_validate:
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                  all_tag_label_ids, all_relation_label_ids)
    ...
    if args.do_train:
        ...
        if args.mode == 1:
            repeat_relations = None
            if args.repeat_relations:
                repeat_relations = (['PNG', 'PNT', 'NNT', 'FNT'], 10)
            train_features = convert_examples_to_features1(
                train_examples, label2id, args.max_seq_length, tokenizer, repeat_relations=repeat_relations)
        # else:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_tag_label_ids,
                                   all_relation_label_ids)
        ...
        for lr in lrs:
            model = XLMRMultiLearning.from_pretrained(
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                progress_bar = tqdm(enumerate(train_batches), total=len(train_batches))
                for step, batch in progress_bar:
                    batch = tuple(t.to(device) for t in batch)
                    input_ids, input_mask, segment_ids, tag_label_ids, relation_label_ids = batch
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        optimizer.zero_grad()
                        global_step += 1
                        progress_bar.set_description(f'loss: {tr_loss / nb_tr_steps}')
                    if args.do_validate and (step + 1) % eval_step == 0:
                        ...
                        preds, result, scores = evaluate(model, device, eval_dataloader,
                                                         eval_tag_ids, eval_relation_ids, num_tag_labels,
                                                         num_relation_labels,
                                                         label2id)
                        model.train()
    ...
    if args.do_eval:
        ...
        preds, result, scores = evaluate(model, device, eval_dataloader,
                                         eval_tag_ids, eval_relation_ids,
                                         num_tag_labels, num_relation_labels, label2id,
                                         compute_scores=False)
        aggregated_results = {}
        for task in ['tag', 'relation']:
            aggregated_results[task] = [
                list(pred[orig_positions]) + [label2id[task]['0' if task.startswith('rel') else 'O']] * (
                        len(ex.sentence) - len(orig_positions)) for
                pred, orig_positions, ex in
                zip(preds[task], eval_orig_positions_map, eval_examples)]
            aggregated_results[f'{task}_scores'] = [
                list(score[orig_positions]) + [0.999] * (len(ex.sentence) - len(orig_positions)) for
                score, orig_positions, ex in
                zip(scores[task], eval_orig_positions_map, eval_examples)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repeat_relations", action='store_true',
                        help="Whether to repeat examples with relations from small classes")
